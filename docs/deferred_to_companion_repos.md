...
| D3 | `aim_is_all_you_need` | [x] **fixed** — warning lifted into both wrappers, column pinned by test | **yes, landed** (P2, plus a swap P2 missed) |
| D4 | `aim_is_all_you_need` | [x] **modelled** — `handling_film_mass()` in `src/bag_state.py` | **yes, landed** (P3, at the 6--25 µm band) |
...
| D8 | `puffsat_impact_simulation` | [x] **regenerated and re-verified** — the document's row is stale, the paper's range is not | no — stale row struck (P0, landed) |
| D9 | `puffsat_impact_simulation` + `aim_is_all_you_need` | [x] **solved properly, and `REF_V_L` moved** — the cliff is 2450 K, not 2570 K | **yes, landed** (P1); P4 was **already applied** under A9 |

**All five paper edits were applied in the paper repository on 2026-08-26 and
`./build.sh` is clean.** See ["What landed"](#what-landed) for what each one
became, including two things P0--P4 did not anticipate: P4 was already in the
paper, and P2 uncovered a swap that had the plate beating the nozzle.
...
Applied to `templateArxiv.tex` and `docs/paper_corrections.md` on 2026-08-26.
`./build.sh --clean` runs clean with no overfull boxes.

| # | what was done |
| --- | --- |
| P0 | Both stale rows struck in `docs/paper_corrections.md`. B2's cold-leg row is replaced by the full 45°/partner-H sweep with **both** axes labelled, and A1's `eta_chem` cell is corrected 0.754 → **0.731** with a do-not-re-apply note. The paper body was not touched, as instructed. |
| P1 | `templateArxiv.tex` now prints **2450 K** and **"thirteen hundred kelvin earlier"**. The sensitivity clause was taken: the paper states that across the 5.5e4--9.7e4 band the crossing moves only 2524 K to 2386 K. It also says outright that the crossing is solved rather than interpolated, and that interpolating the six tabulated rows runs about 120 K high, so a reader cannot re-derive 2570 from the table above it. Cited to `Katz_puffsat_impact_sim_2026`. |
| P2 | The clause naming `f` = 0.818 **was already there** from C1. What was not there, and what P2 did not flag, is that the same sentence had **the two chains swapped** (see below). Both fixed, plus a clause giving the 0.800 figure and the widened 27× ratio so a reader who reruns the target as committed can tell which they are looking at. |
| P3 | `tab:axial_bag` gains an area column and splits `Film` into **`Pressure`** and **`Handling`**, the handling floor quoted across the **6--25 µm band** rather than at a single gauge. `tab:bag_state` gains a second film row taking the greater of the two, so its Jupiter column no longer reads 0 kg. The prose states the floor at 2.4--10.0 kg (1.1--4.7% of the slug) over the 23 m column's 437 m², and the differing scalings (`F` +8% against area +77% from 16 m to 50 m) are now used to support stopping at 23 m. |
| P4 | **Already applied.** `tab:seed_window` was regenerated at `vL` = 7.4e4 under A9 before this document was written. Its σ/`Rm` pairs (1.05/0.1, 63/5.9, 492/45.8, 6987/650) reproduce P4's bold row on every shared temperature, and the `Rm = 361` row P4 warns about is gone. No leak-fraction column exists or was added. Nothing to do. |

### The gauge decision under P3

P3 asked for a citation to be verified before printing a single 12.7 µm gauge. It
was not verified and **the band was printed instead**, which P3 explicitly allows.
The paper's bibliography carries `nasa_echo_ii_1966`, which is **Echo II** and its
~18 µm aluminium--Mylar--aluminium laminate, not Echo 1's 12.7 µm metallised PET.
That cite is used for the weaker claim it does support, that Echo flew orbital
balloons on films in this class. Every P3 conclusion holds across 6--25 µm, and
gauge enters the area scaling only as a constant multiplier.

### The swap P2 did not anticipate

The sentence C1 rewrote read:

> a nozzle on both legs returns \num{4.24e5} against the plate chain's \num{9.52e6}
> at $\eta_{\mathrm{geom}} = 0.9$ and \num{7.49e4} against \num{7.72e5} at $0.8$.
> That is a factor of 22 and a factor of 10, **won** against the incumbent's best
> measured number

**Both pairs were the wrong way round**, so as printed the plate beat the nozzle
22× in a sentence claiming the nozzle won. Three independent checks fix the
ordering, and all three agree:

1. **4.24e5 is the plate.** D3 sources it from `price_chain(cycles, 1.0, 0.818,
   geometric_efficiency=eta_geom)`. The same call at `f` = 0.800 gives 3.547e5,
   which is exactly `tab:space_mortgage_growth`'s own cell at `eta_geom` = 0.9.
   That table is the plate chain by construction.
2. **P2 says so in words.** "The nozzle-over-plate ratio moves 22× against 27×",
   and 9.52e6 / 4.244e5 = 22.4.
3. **The other pair checks independently.** `tab:two_leg_growth` is untolled and
   gives 2.83e7 at `e_1` = `e_2` = 0.8; the paper states the toll shrinks the
   two-leg chain 37×, and 2.83e7 / 37 = 7.65e5, which is the 7.72e5 the sentence
   prints. So 7.72e5 is the nozzle.

Corrected to 9.52e6 against 4.24e5, and 7.72e5 against 7.49e4. Nothing else in
the section moved. The "factor of 22", the "factor of 10", the matched-0.80
comparison at a factor of 36, and the 37× overstatement were all already written
for the correct ordering, which is what made the swap findable.

### One thing left alone

`sec:axial_bag` says the 23 m column costs "a fifth more conductor and
\SI{0.8}{\kilogram} more film". The conductor half checks out against either
baseline (1.19 / 0.99 and 1.19 / 1.00 are both a fifth). The film half does not
reproduce from any row of `tab:axial_bag`: against the sphere it is 1.3 kg
(6.2 − 4.9) and against the 16 m row it is 0.3 kg (6.2 − 5.9). It is outside the
scope of D1--D9, the intended baseline could not be recovered here, and it was
left unchanged rather than guessed at. **Worth a companion check.**
