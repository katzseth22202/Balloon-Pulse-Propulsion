...
outbound 1 AU crossing, and that crossing lands about 130 deg of heliocentric longitude
...
- **Phasing (chosen), realized three ways.** All impulses are PuffSat collisions, not
  ...
  2. *Single-impulse resonant dive* (inject outbound to ~2.7 AU aphelion): only the Earth
     node, but the boost grows 24 to ~41 km/s (+17), dropping the doubling factor below two.
     ~0.82 yr.
  3. *Gravity-assist resonant return* (Venus/Earth flyby): impulse-free, ~1-2 yr, constrained
     by flyby timing.

The three trade network complexity against per-cycle doubling factor against schedule, all at
roughly the same ~0.82 yr cycle. The paper presents all three rather than committing to one.
...
- The companion calculations repo (`katzseth22202/aim_is_all_you_need`) still carries the old
  6-month / under-a-decade figure and should be updated to the 0.82 yr cycle so the two do
  not contradict.
