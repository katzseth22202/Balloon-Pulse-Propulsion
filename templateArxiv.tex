...
\subsection{Tether Tapering And Countering Extreme Drag}
For incoming projectiles, the relative atmospheric drag scales as $v^2 = \frac{(\text{projectile speed})^2}{(\text{trans-lunar PuffSat speed})^2} = \frac{\left(\SI{150}{\kilo\meter\per\second}\right)^2}{\left(\SI{11}{\kilo\meter\per\second}\right)^2} \approx 185$
 times greater than the drag analyzed in \autoref{sec:estimate_cold_gas}.

Because projectiles spend only \SIrange{1}{2}{\second}
at lower altitudes when arriving at such extreme velocities, we may be able to counteract the resulting terminal forces by briefly firing small chemical thrusters and vectoring their thrust. Thermal loads from drag could be mitigated if the projectile skin is thin and highly conductive, with underlying materials—such as water—absorbing heat via convection and phase change (e.g., boiling).

This challenge is significantly reduced if the projectile enters the tube at \SIrange{300}{400}{\kilo\meter} altitude, rather than the \SI{200}{\kilo\meter} baseline used in \autoref{sec:formation_challenges_current_missions}. These higher altitudes slightly exceed the breaking length of commercially available materials like Kevlar or SK99 Dyneema (modified with carbon for creep resistance) \cite{wiki:specific_strength, sk99_tension}.

Given that the PuffSats anchoring the tube travel at only \SI{11}{\kilo\meter\per\second}, we can place one anchor at the lower \SI{200}{\kilo\meter} altitude and connect the upper portion of the tube to a second anchor at higher altitude. Shorter cables, anchored to surface vessels, can serve as guy wires on the lower \SI{20}{\kilo\meter} of the tube exposed to atmospheric winds.  The guy wires can be actively winched to adjust tension and reduce wind-induced loads on the primary structure.  

Despite these mitigations, our current cable designs still lack sufficient engineering safety margin. Tapered cables offer a promising solution.  The mass-optimal cross-sectional tapered cable area at distance $x$ from the anchor scales as $A(x) = Ce^{-\frac{x}{b}},$ where $b$ is the material’s breaking length and $C$ is a constant (see \autoref{sec:breaking_length_derived}).

To maintain an engineering safety factor of 3 across a cable stretched to its full breaking length, the cross-sectional area at the anchor must be roughly 20 times greater than at the point where the cable connects to the tube. Fortunately, this tapering requirement aligns with our existing structural strategy: we already deploy thin control wires every few meters along the tube. By progressively merging and reinforcing these wires as they approach the anchor altitude, we can achieve the necessary increase in cross-sectional area without introducing significant additional mass beyond what’s already budgeted.
