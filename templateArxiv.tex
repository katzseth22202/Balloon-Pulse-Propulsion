...
Because PuffSats are single-use and operate for only one half of a single orbit, we can budget more propellant for aggressive drag and torque correction. For PuffSats, overestimating propellant needs is wise. Unused gas hits the pusher plate, so it is not wasted—though variability in this leftover gas propulsion slightly complicates the intercepting rocket’s trajectory control adjustment between impacts.  The first few PuffSats to intercept the target rocket will have the lowest altitudes (closest to  \SI{200}{\kilo\meter}) before the rocket rises slightly as it accelerates towards orbit (see \autoref{sec:leo_orbit_details}). To optimize propellant usage, we can equip these initial units with a bit more propellant  to compensate for their more challenging trajectory.
