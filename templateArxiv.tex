...
This paper's goal is to enable our progress in the physical world to catch up with our progress online.  Our journey requires applying a single unifying idea that is much simpler than attention.  We call it \textit{aim}.   

Consider a "PuffSat," a single-use microsatellite with a mass of roughly 25–100 kilograms, comparable to the unofficial mass of a 27U CubeSat \cite{cubesat_advanced_standard}.  When triggered, a PuffSat rapidly converts nearly all its mass into a low density gas, generated explosively or by atomizing cold solids or liquids (see \autoref{sec:puffsat_design}). Ultra lightweight microthrusters and electronics let it make small navigation adjustments.
