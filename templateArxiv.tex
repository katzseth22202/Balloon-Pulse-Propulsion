...
Recent missions such as Proba-3 \cite{esa_proba_3} demonstrate that millimeter-scale formation flying works in high-altitude orbits. This indicates that most PuffSat navigation and control tasks can be handled with robust model predictive control (MPC) and Kalman filtering (see \autoref{sec:neural_navigation}).  These classical techniques are data-efficient, easy to interpret, enable clear reasoning about system behavior, and offer formal performance and safety guarantees. For the more demanding conditions near low periapsis, classical control methods can be augmented with neural networks that warm-start control algorithms and learn from residual errors (see \autoref{sec:neural_navigation}).
...
In short, PuffSat navigation can be built primarily on proven estimation and control methods, with machine learning applied when environmental complexity or time constraints push classical tools to their limits.

\subsubsection{Steering Lasers With High Powered Lasers}
