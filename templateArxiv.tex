...
\subsection{Vacuum Exchange And Air Lock Details}
...
A new air lock segment is swapped in between kinetic projectile impacts, providing time to restore the high vacuum offline before its next use. At the bottom of the air lock, the projectile releases enormous heat on contact with steam. Asteroid air-burst simulations suggest the vast majority of this energy is directed in the projectile's direction of travel, away from the air lock and the closest chamber walls \cite{tunguska_downwards}.

If still needed to handle the high initial heat, sacrificial plastic segments can form the bottom part of the air-lock pipe. These plastics should be strong at low temperatures but vaporize easily to avoid creating shrapnel. Their brief exposure to steam before projectile impact can be buffered by a coat of ice, which forestalls heating. To prevent the production of highly toxic gases when they decompose, the plastic should not be halogenated. Cheap polyethylene pipes may satisfy all these requirements.  The pipe can be ribbed for strength and corrugated to stop buckling under the steam's pressure.  These techniques reduce the pipe mass needed to withstand the steam's inward force.  A thicker plastic plug at the bottom of the pipe could drastically quicken projectile disintegration, especially if the projectile primarily contains shock or heat-sensitive explosives. The enormous kinetic energy from the projectile's velocity would be efficiently transferred to the plug on impact, creating intense heat and shockwaves, breaking any debris into smaller shards that vaporize in the steam well away from chamber walls.  An added benefit of these sacrificial segments is that they lengthen the time the air-lock door has to seal before steam can travel far enough to enter the vacuum tube.
...
Operationally, Ceres offers several advantages: adequate solar illumination for power generation, sufficient gravity to support mining infrastructure, shorter transit times than Phoebe, and a low escape velocity. A solar impact trajectory from Ceres requires only about \SI{18}{\kilo\meter\per\second}, significantly less than from Earth. This enables launch cycles with less extreme solar periapsis or greater payload mass per cycle.

Achieving millimeter-accuracy PuffSat interceptions around Ceres may prove easier than in most other key solar system locations--especially compared to near the Earth or the Sun. At high but easily reachable orbits above Ceres, PuffSats would experience very little unmodeled drag, magnetism, or gravitational pull. Solar radiation pressure is less of a factor. While Jupiter's influence might be significant, it would also be highly predictable. 

Rather than relying on active solar tracking in the dim polar sunlight, energy can be harvested from sun-synchronous tracking orbits around Ceres. In orbit, solar panels and lenses avoid the thermal cycling challenges posed by Ceres' night. Thanks to its low gravity, mined carbon and water can be lofted into orbit with minimal propulsion, then converted into fuel at orbital solar farms. This fuel can power return trips to the surface and drive mining operations.

Unlike Phoebe, Ceres lacks a nearby planetary body for Oberth maneuvers. However, PuffSats returning from solar periapsis can deliver momentum for high-$\Delta V$ trajectories—either back to Earth or towards close solar periapsis. These returning PuffSats also offer an alternative to solar power, explosively excavating rocky overburden or melting large subsurface ice for easy pumping to fill PuffSats.
