...
Two control jobs shape what replaces the tank. Sustained drag and torque cancellation needs throttleable thrust near \SI{400}{\milli\newton}, the peak drag force estimated from GOCE modeling (see \autoref{sec:estimate_cold_gas}), held for minutes. This favors a continuous feed of liquid or generated gas. Brief attitude and divert corrections need fast, discrete impulses, which favor a bank of pre-measured solid charges. A single PuffSat can carry more than one of the forms below, matched to these jobs.
...
If the plumbing and support hardware for this microfluidic system weigh no more than \SI{50}{\gram}, the total dry mass would likely be lower than that of a standard \SI{400}{\gram} nitrogen tank. The compact geometry also reduces drag and makes ejection ahead of pusher contact more straightforward (see \autoref{sec:coordinator_node_dry_mass_disposal}). As the thin polymer bladder skin fragments travel through the gas plume toward the pusher, they are decelerated and ablated, likely limiting impact damage to the pusher's ablative layers.  The increased complexity of microfluidics introduces the risk of catastrophic PuffSat failures. However, the system must already be resilient to PuffSat losses, given many are used on each launch.
