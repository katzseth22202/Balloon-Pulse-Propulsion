...
Each balloon carries up to 100 grams of solid mass, including sensors, power systems, microthrusters, and low-bandwidth communication links to the coordinator nodes. To prevent this mass from impacting the rocket, we propose several disposal strategies
\begin{itemize}
    \item \textbf{Active Elimination:} Equipment on the target rocket could vaporize the solids via electric discharges, directed energy beams, or reactive sprays.
...
In 2001, Gerald Nordley proposed using external propulsion from the  momentum transfer by self steering hyper velocity pellets impacting a pusher plate \cite{nordley2001interstellar}.   Nordley essentially proposed adding guidance to the pellet propulsion concept originally explored by Clifford Singer \cite{singer1979interstellar}.  Each of these concepts required implausibly powerful mass drivers capable of propelling macroscopic objects to extreme velocities.  Jordan Kare's SailBeam \cite{kare2001sailbeam} replaced the mass driver with solar or laser driven propulsion.  Kare also proposed vaporizing the sail on impact and using a magnetic pusher plate.   

Since pellet stream propulsion proposals often focus on interstellar missions, economic feasibility was secondary.  Balloon pulse propulsion can use expensive mass drivers or light sails for propulsion, we focus primarily on achieving high speeds with orbital manuevers, which costs little.   Our balloons can weigh kilograms, and are mostly comprised of environmentally benign gases like water vapor.   This means they generate substantially less pollution for a given \Delta v, which makes them practical for large scale application placing objects in orbit.  Water and other volatiles  also very cheap compared to electronics, and can be mined throughout the solar system.  Larger balloons can afford heavier electronics than guided pellets.  Given current technology, it's unclear the snowflake sized propellant-less self steering Nordley envisioned would be realistically achievable.  While balloons can use the magnetic pusher plates Nordley envisioned, this is probably more complicated than simply allowing the balloon to impact the pusher plate.  Since the balloons are gaseous, vaporizing impacting components is potentially simpler than prior applications.
