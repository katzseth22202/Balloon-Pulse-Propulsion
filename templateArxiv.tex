...
\section{Deriving Effective $\Delta v$ for idealized 100\% efficient prograde/retrograde balloon thrust}\label{sec:dv_effective}
Suppose a retrograde balloon with $m_rb$ and a prograde balloon with $m_{pb}$ collide in a combustion chamber.  We want to find maximum $\Delta v$ when engine efficiency is 100\% and for simplicity we say 
\begin{equation}
...
Let $v_g$ be gas exhaust velocity expelled from the rocket.  Applying mass from \autoref{eq:mass_is_1}
 ...
 \begin{equation}
 v_g = 2v\sqrt{m_{rb}} \label{eq:vg_result}
 \end{equation}
