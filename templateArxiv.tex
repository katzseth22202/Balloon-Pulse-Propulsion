...
Operating at a lower interception altitude (\SI{200}{\kilo\meter}) introduces stronger and more variable drag perturbations than at VISORS' \SI{600}{\kilo\meter} orbit.  GOCE, a substantially larger satellite, achieved full drag compensation at \SI{250}{\kilo\meter} using a streamlined aerodynamic profile coupled with ion thrusters \cite{goce_dragfree}. Though electric propulsion for CubeSats is actively being developed \cite{busek_iodine}, adapting GOCE's ion propulsion system for a PuffSat might still be unrealistic due to its mass and power constraints.  However, PuffSats could be stretched like GOCE to reduce drag. Torque could be mitigated by modest spin stabilization or GOCE-like fins and combined with cold gas or small chemical thrusters for fine control. Because PuffSats are single-use and operate for only one half of a single orbit, we can budget more propellant for aggressive drag and torque correction.   
