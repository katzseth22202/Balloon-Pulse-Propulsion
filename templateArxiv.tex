...
\]
Since \( dm_b \) is infinitesimal, we neglect higher-order terms involving \( (dm_b)^2 \). Rearrange:
\[
...
\]
Expand \( (v_r + dv_r)^2 \approx v_r^2 + 2 v_r dv_r \) (neglecting \( (dv_r)^2 \)), and since \( dm_b \) is small, the energy equation becomes:
\[
...
\]
\[
m_r v_r dv_r \approx dm_b \frac{v_b^2 - (v_b')^2}{2} \tag{2}
\]
Since \( v_b^2 - (v_b')^2 = (v_b - v_b')(v_b + v_b') \), we need \( v_b' \).
\subsection{Velocity change of \(m_r\)}
Substitute \( v_b' = 2 v_r - v_b \) into the momentum equation (1):
\[
...
\]
\[
dv_r = \frac{2 (v_b - v_r)}{m_r} dm_b \tag{4}
\]
...
The velocity of \( m_r \) changes from \( v_{ri} \) to \( v_{rf} \) as more balloons collide. Integrate equation (4):
\[
...
\]
For the right-hand side, treat \( v_r \) as a function of the accumulated balloon mass \( m_b \). However, we need to express \( v_r \) in terms of \( m_b \). From equation (4):
\[
\frac{dv_r}{v_b - v_r} = \frac{2 dm_b}{m_r}
\]
Rewrite:
\[
...
\]
\[
m_b = \frac{m_r}{2} \ln \left| \frac{v_b - v_{ri}}{v_b - v_{rf}} \right| \tag{5}
\]
...
\]
- If \( v_{ri} \) or \( v_{rf} > v_b \), the relative velocity \( v_b - v_r \) changes sign, and we must ensure the physical context is consistent (e.g., balloons impart momentum in the direction that reduces \( v_r \) toward \( v_{rf} \)).

We can then solve for the total balloon to rocket ratio, with a fudge factor e between 0 and 1 to account for the coefficient of restitution and imperfect spread of gaseous volatiles off the pusher plate \[
\frac{m_r}{m_b} = \frac{2e}{ln(\frac{v_b-v_{ri}}{v_b-v_{rf}})}
\]
