...
Operating at a lower interception altitude (\SI{200}{\kilo\meter}) introduces stronger and more variable drag perturbations than at VISORS' \SI{600}{\kilo\meter} orbit.  VISORS has a near circular sun-synchronous orbit so is continously exposed to peak drag, while PuffSats benefit from only spending minutes below this altitude.  While PuffSats traverse low altitudes at higher speeds than VISORS, their short time at these heights limits the accumulation of drag-induced drift.  GOCE, a substantially larger satellite, achieved full drag compensation at \SI{250}{\kilo\meter} using a streamlined aerodynamic profile coupled with ion thrusters \cite{goce_dragfree}. Though electric propulsion for CubeSats is actively being developed \cite{busek_iodine}, adapting GOCE's ion propulsion system for a PuffSat might still be unrealistic due to its mass and power constraints.  However, PuffSats could be stretched like GOCE to reduce drag.

GOCE demonstrated that small accelerometers can accurately measure atmospheric drag, enabling precise corrective maneuvers. Torque could be mitigated by modest spin stabilization or GOCE-like fins, combined with cold gas or small chemical thrusters for fine control.

Because PuffSats are single-use and operate for only one half of a single orbit, we can budget more propellant for aggressive drag and torque correction. For PuffSats, overestimating propellant needs is wise. Unused gas hits the pusher plate, so it is not wasted—though variability in this leftover gas propulsion slightly complicates the intercepting rocket’s trajectory control adjustment between impacts.

Based on crude GOCE-based modeling discussed in \autoref{sec:estimate_gold_gas}, our PuffSat uses only \SI{400}{\gram} of gas, and maximum forces of approximately \SI{400}{\milli\newton} are sufficient to cancel drag forces below VISORS' altitude before impact. This is likely acceptable, since propellant represents less than 2\% of total PuffSat mass under this "back of the napkin" propellant modeling. 

Cold gas thrusters, like all PuffSat components, must minimize dry mass to reduce impact risk with the pusher plate (see \autoref{sec:coordinator_node_dry_mass_disposal}). One option is to store nitrogen in \SI{100}{\micro\meter}-thin, low-density, high-tensile bladders rather than rigid canisters.   The bladders would travel through the generated gas plume on their way to the pusher plate, both ablating and decelerating them so that pusher plate impact produces no damage beyond the pusher's thin ablative layer (see \autoref{sec:lightweight_pusher_plates}). 

The first few PuffSats to intercept the target rocket will have the lowest altitudes (closest to  \SI{200}{\kilo\meter}) before the rocket rises slightly as it accelerates towards orbit (see \autoref{sec:leo_orbit_details}). To optimize propellant usage, we can equip these initial units with a bit more propellant  to compensate for their more challenging trajectory.
...
Model GOCE as a cylinder with diameter \(D_0=\SI{1.0}{\m}\) and length \(L_0=\SI{4.0}{\m}\).  Then the radius \(r_0=\tfrac{D_0}{2}=\SI{0.5}{\m}\), and
\[
