...
Operating at a lower interception altitude (\SI{200}{\kilo\meter}) introduces stronger and more variable drag perturbations than at VISORS’s \SI{600}{\kilo\meter} orbit.  GOCE, a substantially larger satellite, achieved full drag compensation at \SI{250}{\kilo\meter} using a streamlined aerodynamic profile coupled with ion thrusters \cite{goce_dragfree}. GOCE's ion propulsion system is likely too massive and power-intensive for PuffSats, unfortunately. However, PuffSats could be stretched even more than GOCE to cut through the tenuous exosphere.  For idealized modeling purposes, consider an explosive PuffSat (see \autoref{sec:explosive_puffsat}) shaped as a simple cylinder of \SI{25}{\kilo\gram} ANFO with a \SI{5}{\centi\meter} radius.  This PuffSat would be about \SI{3.8}{\meter} long.  Elongation can induce tumbling, but we can offset this tendency by gently spinning the PuffSat.  Inspired by GOCE, we also use fins to stabilize orientation and potentially steer through drag perturbations at the lowest altitudes, modestly reducing performance demands on cold gas or chemical thrusters. Since PuffSats operate for only a single orbit, a greater fraction of the micro-thruster propellant budget can be dedicated to aggressive drag correction.  Since the first few PuffSats intercept the target rocket before it starts accelerating towards orbit, their impacts might occur at the lowest, most challenging altitudes right at \SI{200}{\kilo\meter} (see \autoref{sec:leo_orbit_details}). We can optimize by allocating these first PuffSats a bit more fuel and more powerful thrusters. 
