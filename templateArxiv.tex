...
If attention was all we needed to build AI, this paper argues that precision aim is all we need to solve grand challenges in the physical world.

Specifically, we propose a space propulsion method driven  by high-precision, coordinated orbital intercepts.  Consider a "PuffSat," a single-use microsatellite with a mass of roughly 25–100 kilograms, comparable to the unofficial mass of a 27U CubeSat \cite{cubesat_advanced_standard}.  When triggered, a PuffSat rapidly converts nearly all its mass into a low density gas, generated explosively or by atomizing cold solids or liquids (see \autoref{sec:puffsat_design}). Ultra lightweight microthrusters and electronics let it make small navigation adjustments.
...
The optimal PuffSat design depends on the launch scenario and mission requirements: explosive PuffSats fit terrestrial launch, while the ice and liquid-oxygen designs that follow use volatiles mined in space (see \autoref{sec:lunar_rockets_no_fuel} and \autoref{sec:lunar_mining}).
...
PuffSats filled with lunar oxygen (see \autoref{sec:lunar_mining}) must also atomize before reaching the pusher plate. To retain liquid oxygen during orbital transit, these PuffSats require thicker skins engineered for elevated internal pressures, cryotolerance, and thermal insulation. To minimize boiling of the highly pressurized LOX (liquid oxygen), PuffSats use high-reflectivity coatings such as emerging "solar white" yttrium oxide composites \cite{nasa_solar_white_y2o3} (see \autoref{sec:epstein_drives}).  These coatings are backed by emissive layers that radiate residual heat into space, maintaining cryogenic temperatures without active cooling.
