...
\section{One Hiroshima Per Second. The World Set Free} 
In \autoref{sec:no_isru_rocket}, we propelled balloons to \SI{150}{\kilo\meter\per\second} at \SI{1}{\astronomicalunit} using solar gravity and externally pulsed propulsion. A \SI{60}{\tera\watt} power source, which is equivalent to one Hiroshima bomb per second \cite{hiroshima}, requires harnessing the kinetic energy of just \SI{5.3}{\tonne\per\second} of propulsion mass at these speeds. For context, humanity's total power consumption is approximately \SI{20}{\tera\watt} \cite{owid-energy-production-consumption}. Given that typical power plants are roughly one-third efficient, a single one of these power sources could meet the energy needs of all human civilization.

Remarkably, this concept evokes H.G. Wells's prophetic vision in his 1913 novel, \textit{The World Set Free}, where he imagined atomic bombs as a "blazing continual explosion" \cite{wells1914world}.  We will now explore how to convert the kinetic energy generated by such externally pulsed propulsion into a terrestrial power source.

\subsection{Straw Ways To Heaven}\label{sec:death_star}
For kinetic projectile energy to reach Earth, the projectiles need to survive through the atmosphere.  One way to do this is to build a vacuum tube from Earth's surface to space aligned with the projectile's path as depicted in \autoref{fig:straw_way_to_heaven}.
