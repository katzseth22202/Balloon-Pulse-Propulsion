...
Orbital disturbances are predictable over short timeframes. We can use this to our advantage. The probability distributions for these low-altitude disturbances are consistent for minutes at a time. This means we can use real-time observations of a PuffSat's perturbed path to refine trajectory predictions for subsequent PuffSats. 

To optimize the process, we can deploy "scout" picosatellites a few minutes ahead of the main formation. These scouts would have small, inflatable booms, similar to the one on the GASPACS 1U CubeSat \cite{ellis2014gaspacs}, to increase their surface area and maximize aerodynamic drag. This increased drag creates a more significant orbital perturbation on the low-mass scout, which can be measured with onboard accelerometers. Despite their minimal mass, these scouts can carry essential instruments, including sensors, power, micro-thrusters, and communications equipment. When the scouts broadcast the measured drag data, they give the main formation a few minutes' warning to execute preemptive orbital corrections. The scouts are built with minimal components and are designed to burn up completely on re-entry (see \autoref{sec:handling_space_debris}).
...
\subsubsection{What to Expect When You're Expecting Bodily G-Force \cite{expecting_pregnancy_book}} \label{sec:pregnant_women}
While aerospace medicine has advanced significantly, little data exist on g-force tolerance in pregnant women and infants. This gap complicates the design of mainstream suborbital transit systems. One potential mitigation is to seat passengers so g-forces act transversely—across the body—rather than along the head-to-foot axis. In this orientation, the force moves across the torso instead of head-to-toe, a configuration generally better tolerated as it reduces blood pooling in the lower extremities. However, the author is not aware of any studies confirming safety in vulnerable populations. Rigorous, multi-stage trials—beginning with non-human primates and progressing to controlled human studies—would be required before such a protocol could be validated for operational use.
