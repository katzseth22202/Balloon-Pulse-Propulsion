...
\begin{table}[!htpb] % tabularx should be inside a table environment
    ...
    \begin{tabularx}{\textwidth}{|p{4em}|p{4em}|p{4em}|p{6em}|L|}\hline
        ...
        10.916 & 69.277 & 0 & 9.2 & Balloons approach Earth from Jupiter and push a rocket into an elliptical orbit \\\hline
...
A simpler way to bootstrap an exponential launch cycle is to use the retrograde Hohmann trajectory from Jupiter described in \autoref{sec:jupiter_gravity_initial} but without close solar approaches.   Balloons returning from Jupiter can push perhaps 8 times their weight into a highly eccentric orbit around Earth.   A small conventional burn coupled with solar electric engines and gravity assists with the inner planets can return this payload to Jupiter and restart the cycle.   This multiplies payload capacity by a factor of 8 roughly every 6 years.   The growth is slow compared to other proposals but may provide sufficient launch capacity to supply the satellite market in a little more than a decade. 

We can also combine this Jupiter launch cycle with reusable rockets or the lunar ices based cycle of \autoref{sec:lunar_rockets_no_fuel}.  This increases our initial payload sent to Jupiter, so that the slow exponential growth rate doesn't limit us from using it for larger markets like intercity transport or energy generation.
