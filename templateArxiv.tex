...
SpaceX's Starship may be the first fully reusable rocket. At scale, this could reduce space flight costs.   However, the rocket's large payload size is probably too big for today's space industry needs. For example, in \textit{The New York Times}
...
\begin{itemize}
\item \textbf{Air-breathing Scramjets (Optimistic Scenario):} If economical passenger scramjets become feasible, rockets could accelerate to around Mach 7 \cite{nasa_x43a} before a steep climb, eliminating the need for liquid oxygen. This would also reduce propellant mass. Unfortunately, the extreme stress on the airframe makes suborbital rockets more economically viable for the foreseeable future.
...
A formation of propulsion PuffSats launched in an eccentric Keplerian orbit pushes the rocket plane onto a ballistic arc. That arc lies in a plane through the Earth's center, so its ground path is a great circle. This does not restrict which cities are reachable. Any two points on the globe share a great circle, and the shorter arc of that circle is the geodesic between them \cite{docarmo_diffgeo, bowditch_navigator}. The formation's orbital plane can therefore be chosen to pass over both the departure airport and the destination. Earth's rotation and oblateness do not change this. The plane is aimed at the destination's position at arrival time. The $J_2$ oblateness perturbation over a single suborbital arc is deterministic. Both effects go into the launch targeting instead of an in-flight correction \cite{vallado_astrodynamics}. Connecting departure and arrival at a fixed flight time is a two-point boundary value problem with a Keplerian solution \cite{battin_astrodynamics}. A single formation designed for the trip can therefore place the rocket plane on target without a dedicated plane-change burn. This removes an onboard maneuvering stage and its propellant mass. A small terminal trim may still be carried to absorb navigation error, supplied either by a second formation or by the reentry pass described next. To avoid a sonic boom upon descent, the plane must decelerate before atmospheric reentry. This deceleration could be provided by another set of propulsion PuffSats traveling in a retrograde orbit relative to the plane. These reentry PuffSats could be in a circular low Earth orbit rather than a high eccentricity orbit, reducing the required mass.
...
Unfortunately, I found no studies directly evaluating the safety of even moderate (2 g) $Gx$ acceleration for pregnant women or infants. We must test safety protocols with multi-stage trials.   These trials can initially test on mice and then progress to non-human primates.  Since human babies have larger brains, only tests on human subjects can truly confirm they are not harmed when following acceleration mitigation procedures.  If we can't establish that suborbital flight is safe for these groups, they must be excluded from suborbital flights.  
