...
Centimeter-accuracy CubeSat formation flying is rapidly advancing. The European Space Agency's (ESA) Proba-3 mission recently demonstrated this capability by creating artificial solar eclipses with a precise two satellite formation \cite{esa_proba_3}. The Stanford Space Rendezvous Laboratory's VISORS mission \cite{guffanti2023autonomous}, nearing flight readiness (as of August 2025), will showcase centimeter-precision formation flying with 6U CubeSats for \SI{10}{\second} durations at a \SI{600}{\kilo\meter} altitude \cite{visors_formation_flying}.

Our hypervelocity PuffSats employ formation-flying algorithms for precise positioning, but with less stringent requirements than VISORS or Proba-3. Those missions demanded sub-arcsecond pointing for imaging, whereas PuffSats can tolerate wider rotational misalignments and still satisfy mission objectives. Similarly, a formation where 90% of units remain within \SI{5}{\centi\meter} of the pusher-plate center may be acceptable, relaxing the lateral control challenge.

Operating at a lower interception altitude (\SI{200}{\kilo\meter}) introduces stronger and more variable drag perturbations than at VISORS’s \SI{600}{\kilo\meter} orbit. However, PuffSats operate for only a single orbit, allowing a greater fraction of the micro-thruster budget to be dedicated to aggressive drag correction. To manage this environment, a hybrid propulsion system may be required: chemical micro-thrusters for primary maneuvers and cold-gas actuators for fine adjustments. Since the first few PuffSats intercept the target rocket before it starts accelerating towards orbit, their impacts might occur at slightly lower, most challenging altitudes (see \autoref{sec:leo_orbit_details}). We can optimize by allocating these first PuffSats a bit more fuel and more powerful thrusters. Aerodynamic shaping of the PuffSats can further reduce sensitivity to drag fluctuations, though variability at this altitude remains a major source of uncertainty.

Atmospheric drag fluctuations remain similar over timescales of tens of seconds at these altitudes, allowing us to refine a PuffSat’s trajectory using observational data from units ahead of it. To further capitalize on this, we propose deploying lightweight “scout” picosatellites roughly \SI{30}{\second}  ahead of the main formation. For better measurements, we can amplify drag perturbations by equipping scouts with inflatable booms like those used on the GASPACS 1U CubeSat \cite{ellis2014gaspacs}. Onboard accelerometers and other instruments measure this drag, and the results are relayed to the trailing PuffSats.  The PuffSats can then execute preemptive orbital corrections. Despite their small mass, the scouts can carry essential subsystems--sensors, micro-thrusters, power, and communications--and are designed for complete burn-up on re-entry (see \autoref{sec:handling_space_debris}). Even with lifetimes in the tens of seconds, they provide valuable early warning in a highly-volatile regime.
...
As shown in \autoref{fig:explosive_puffsat}, each PuffSat carries an onboard detonation mechanism. To ensure reliable detonation, the rocket also releases a submillimeter-thick layer of volatile material, such as water ice, a few meters in front of the pusher plate just before each PuffSat's arrival. The resulting impact shock ensures the PuffSat’s explosive charge detonates. The ice sheet’s geometry guarantees that a PuffSat cannot hit the rocket without first passing through it.  The ice sheet can be a mostly empty grid or lattice, reducing areal density significantly.  For example, rectangular ice wires \SI{250}{\micro\meter} in a square grid with \SI{10}{\centi\meter} spacing weighs only $\SI{100}{wires\per\square\meter} \times \SI{250}{\micro\meter}^2 \times \SI{917}{ice\kilogram\per\cubic\meter} \approx \SI{6}{\gram\per\square\meter}$.
