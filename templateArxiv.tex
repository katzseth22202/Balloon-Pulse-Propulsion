...
The probability distributions for low-altitude orbital disturbances are similar over minutes-long time frames. This means real-time observations of each PuffSat’s perturbed path can refine the initial trajectory guesses used to control later PuffSats.  One or more "scout" PuffSats are also deployed a few minutes ahead of the formation. The scouts don't carry mass to gasify on pusher plate impact, but are designed with a slightly larger dry mass to accommodate essential instruments. The scouts utilize tools like accelerometers to detect and broadcast any micro-perturbations they encounter. Their overall light weight makes them exceptionally sensitive to these subtle changes.  Scout data allows the main formation to optimize its trajectory a few minutes ahead of time. Despite having a slightly larger dry mass for their instruments and telemetry, the scouts must still be constructed with minimal component sizes to guarantee they fully burn up upon re-entry (see \autoref{sec:handling_space_debris}). If control remains impractical, we can simplify by increasing the PuffSat interception altitude.  If we raise interception to \SI{300}{\kilo\meter}, a reusable suborbital target rocket (see \autoref{sec:200_mile_high}) would require somewhat higher propellant mass fractions, suffers marginally greater re-entry heating, and benefits slightly less from PuffSat velocity.   However, raising interception height may be an acceptable compromise if lower altitudes near \SI{200}{\kilo\meter} simply prove unmanageable. 
