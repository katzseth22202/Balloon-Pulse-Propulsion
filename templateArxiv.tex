...
The sections that follow open with the business case (\autoref{sec:business_case}), then explain how PuffSats work (\autoref{sec:how_puffsats_work}) and survey applications. The supporting engineering analysis is gathered in \autoref{sec:engineering} for readers who want the detailed feasibility work. 

\section{To The Moon by Smart Gas Balloon:  The Business Case For PuffSat Pulsed Propulsion}\label{sec:business_case}   

We open with the near-term business case for PuffSat pulsed propulsion, then describe how the system works in \autoref{sec:how_puffsats_work}.
...
Having made the commercial case, we now describe how a PuffSat turns its mass into gas and transfers momentum to a target rocket. This section covers the PuffSat designs, how the concept compares to earlier pellet-beam proposals, and the rocket-to-PuffSat mass ratio that sets the payload budget. The detailed feasibility analysis is gathered in \autoref{sec:engineering} for readers who want it. That analysis covers drag budgets, guidance software, debris handling, deployment, and pusher-plate materials, and can be skipped on a first pass.
...
\section{Engineering the System}\label{sec:engineering}

This section gathers the engineering feasibility analysis behind the applications above. Earlier sections point here through cross-references when they rely on these results, so it can be read in any order or skipped on a first pass. The topics are PuffSat formation flying and drag, the coordinator-node architecture, guidance and estimation software, space-debris handling, low Earth orbit deployment, and pusher-plate design.
