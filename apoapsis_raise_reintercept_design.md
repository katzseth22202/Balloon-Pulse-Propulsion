...
**Status:** Downgraded to a brief-paragraph alternative (see §9 / §9a, 2026-07-13). The trajectory and the finite-thrust check still stand, but the Jupiter gravity-assist return, which is the paper's *existing* `sec:jupiter_only_growth` (~69 km/s, >9× per collision), beats this scenario on propellant, closing speed, per-cycle mass gain, and deep-space hardware. This scenario is now the nearest-term, gravity-assist-free, no-outer-system demonstrator, not the headline low-infrastructure option. **Folded into the paper 2026-07-13**: `\paragraph{Loops without the dive.}` closing `sec:earth_reintercept` (Jupiter return presented first, apoapsis-raise second, with the f ≈ 0.52 break-even caveat) plus a `tab:mass_scenarios` row (11.009 / 24.059 / 0 / 2.616 → `\autoref{sec:earth_reintercept}`). Calc repo port merged as `d821868` (`apoapsis_raise_reintercept` / `_economics` / `_finite_burn`, catalog row, pinning tests; repo uses the self-consistent v_rf = 11.009 km/s, giving ratio 2.616 / growth 1.545 / millionfold 53.7 yr vs. this doc's flat-11.0 values 2.619 / 1.547 / 53.6).
**Date:** 2026-07-13
...
**Intended home:** this paper repo, now as a brief paragraph in `sec:earth_reintercept`, paired with the Jupiter gravity-assist return (§9) as the two low-solar-infrastructure alternatives to the main solar-dive loop. Folded into the paper text 2026-07-13 (see Status).
